...
**`structured_daily` tasks:** `bronze_structured` → `silver_structured` → `load_silver` → `dbt_marts`
...
- Load: `--dataset price,index_price,price_board`
- dbt: `--select +mart_stock_daily +mart_price_board +mart_market_overview`
