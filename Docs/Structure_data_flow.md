...
- **Rate limit:** `rate_limit_rpm` (mặc định 50 req/phút) + `inter_request_delay_sec` giữa từng mã (company, financial_ratio).
...
|---|---|
| **price** | Toàn bộ listing universe sau filter (~1,500+ mã nếu `use_listing_as_universe=True`); 50 mã cố định nếu `False` |
| **index** | `VNINDEX`, `VN30`, `HNXINDEX`, `HNX30`, `UPCOMINDEX` |
| **listing** | Toàn bộ mã trên HNX, HOSE, UPCOM (~3000+ dòng, snapshot) |
| **company** | Cùng danh sách `cfg.tickers` sau khi load từ listing (hoặc tickers cố định) |
| **financial_ratio** | Subset `cfg.tickers` theo `financial_ratio_exchange_filter` (mặc định HOSE/HNX) |
| **price_board** | Config class mặc định = **full listing** (`price_board_use_listing_universe=True`, batched → **một** Bronze `snapshot_at`/run); notebook hiện tại đặt `PRICE_BOARD_MODE=watchlist_50` nên backfill thường lấy snapshot 50 mã |
...
3. **`UNIVERSE_MODE`** (cell config):
   - **`demo_50`:** 50 mã cố định — test nhanh.
   - **`full_listing`:** ~1,500+ mã từ Bronze listing sau ô `ingest_listing` + `sync_cfg_tickers` (production backfill).
4. **`RUN_PROFILE`**:
   - **`backfill`:** `use_incremental_window=False` → fetch full ~`years_back` năm (mặc định 5).
   - **`daily_incremental`:** `use_incremental_window=True`, `incremental_window_days=1` → incremental theo watermark/cửa sổ (giống mặc định code / DAG).
5. **`RUN_ONE_SHOT_PIPELINE`:** `True` → một cell gọi `run_structure_ingestion_pipeline` hoặc `run_structure_full_ingestion_pipeline` (tùy `INCLUDE_FINANCIAL_RATIO`); `False` → chạy từng ô `ingest_*`.
6. Gán `cfg.run_partition` (timestamp mỗi lần chạy Jupyter) nếu cần.

Code mặc định (`IngestionConfig()`): `use_listing_as_universe=True`, `listing_exchange_filter=["HOSE","HNX"]`, `rate_limit_rpm=50`, `use_incremental_window=True` — **không** cần notebook khi chạy DAG.

### 4.2. Airflow DAG (implemented)

| DAG | Schedule (ICT) | File | Pipeline |
|---|---|---|---|
| `structured_daily` | T2–T6 16:30 | `docker/airflow/dags/structured_daily.py` | bronze `price + index + price_board` → silver → load → dbt |
| `structured_monthly` | Ngày 1 hàng tháng 17:00 | `docker/airflow/dags/structured_monthly.py` | bronze `listing + company + financial_ratio` → silver → load → dbt |

**`structured_daily` tasks:** `bronze_structured` → `silver_price` → `silver_index_price` → `silver_price_board` → `load_silver` → `dbt_marts`
...
- Bronze: `run_structure_ingestion_pipeline(IngestionConfig())`, `include_prices=False`, `include_indices=False`, `include_price_board=False`
- Silver: lần lượt CLI `--dataset listing`, `company`, `financial_ratio`
- Load: `--dataset listing,company,financial_ratio`
...
### 4.3. Tóm tắt backfill -> DAG theo dataset

| Dataset | Notebook backfill | DAG mặc định sau backfill | Bronze | Silver / DB |
|---|---|---|---|---|
| `listing` | Full listing snapshot, sau đó dùng filter `HOSE/HNX` để tạo universe chạy structured | `structured_monthly` refresh 1 tháng 1 lần | `listing/master/listing.parquet` | `silver/listing/current/` -> `silver.listing` |
| `price` | Full 5 năm cho toàn bộ ticker lấy từ listing HOSE/HNX | `structured_daily` incremental; mặc định `watchlist50`, có thể đổi qua `STRUCTURED_DAG_UNIVERSE=listing` | `price/year=<YYYY>/month=<MM>/<TICKER>.parquet` | `silver/price/trading_date=<YYYY-MM-DD>/` -> `silver.price` |
| `index_price` | Full 5 năm cho 5 chỉ số cố định | `structured_daily` incremental theo ngày | `index/year=<YYYY>/month=<MM>/<INDEX>.parquet` | `silver/index_price/trading_date=<YYYY-MM-DD>/` -> `silver.index_price` |
| `company` | Snapshot cho toàn bộ ticker trong listing universe | `structured_monthly` refresh 1 tháng 1 lần | `company/snapshots/snapshot_date=<run_date>/company_overview.parquet` | `silver/company/current/` -> `silver.company` |
| `financial_ratio` | Full HOSE/HNX từ listing universe | `structured_monthly` refresh 1 tháng 1 lần | `financial_ratio/snapshot_date=<run_date>/<TICKER>.parquet` | `silver/financial_ratio/period_type=<...>/year=<YYYY>/` -> `silver.financial_ratio` |
| `price_board` | Notebook hiện tại mặc định chỉ lấy snapshot hiện tại cho `watchlist50` | `structured_daily` snapshot current cho `watchlist50` mỗi ngày | `price_board/snapshot_at=<timestamp>/PRICE_BOARD_SNAPSHOT.parquet` | `silver/price_board/trading_date=<YYYY-MM-DD>/` -> `silver.price_board` |

Điểm quan trọng:

- `price` và `index_price` là hai dataset structured có incremental thật theo watermark `trading_date`.
- `listing`, `company`, `financial_ratio` là snapshot-style dataset: DAG monthly sẽ refresh lại snapshot mới, không append theo ngày giao dịch như OHLCV.
- `price_board` là snapshot intraday tại thời điểm chạy; Silver giữ bản mới nhất theo `symbol + trading_date`.

### 4.4. Chạy từ Python (không notebook)
...
- Điều phối: `_ingest_price_board_snapshot`; mã từ `cfg.price_board_tickers` (= `DEFAULT_PRICE_BOARD_TICKERS` trong `config.py`), **độc lập** `cfg.tickers` / listing universe.
- Một request `price_board(symbols_list=…)` cho watchlist (mặc định 50 mã); **một** partition Bronze `snapshot_at=<YYYY-MM-DDTHH-MM-SS>` (UTC) / lần chạy.
- File: `PRICE_BOARD_SNAPSHOT.parquet` (+ cột `snapshot_at`, `data_source`).

**Mặc định:** full listing HOSE/HNX, fetch theo batch 50 mã, **gộp một** folder `snapshot_at=*`/run. Watchlist 50 mã: `price_board_use_listing_universe=False`, `price_board_batched=False`. Silver dedupe `symbol + trading_date`.
...
### 5.8. Listing-as-universe (`use_listing_as_universe=True`)

Khi `use_listing_as_universe=True` (mặc định), pipeline tự động:

1. Chạy `ingest_listing()` để refresh Bronze listing.
2. Đọc `listing/master/listing.parquet`, áp filter `listing_exchange_filter`
   và `listing_security_type_filter`.
3. Gán danh sách symbol đã filter làm `cfg.tickers` cho toàn pipeline.

Điều này cho phép pipeline tự mở rộng ticker universe khi có mã mới niêm yết
mà không cần sửa config thủ công.

**Financial ratio** dùng filter riêng (`financial_ratio_exchange_filter`,
mặc định `["HOSE", "HNX"]`) vì data UPCOM thường sparse và làm chậm pipeline.

Để giữ 50 mã cố định như trước: `use_listing_as_universe=False`.

### 5.9. Batch ingestion (price và price_board)

Với ~1,500 tickers, gọi API cho toàn bộ trong một lần dễ timeout / rate limit:

- **`price`:** batch `price_batch_size` (mặc định 100), mỗi batch ghi OHLCV riêng.
- **`price_board` (mặc định):** listing universe, `price_board_batched=True`, batch 50 mã → concat, **một** `snapshot_at`/run.
- **Watchlist 50 mã:** `price_board_use_listing_universe=False`, `price_board_batched=False`.
- Nghỉ `delay_between_batches_sec` giữa các batch (price OHLCV và price_board batched).

**Lỗi một batch không dừng pipeline.** Nếu batch raise exception, pipeline
log WARNING và tiếp tục batch tiếp theo. Kết quả pipeline trả về danh sách
batch nào failed để operator xem xét.
...
Mặc định loader đọc **toàn bộ** file parquet khớp glob mỗi lần chạy (full refresh từ filesystem). Với `--latest-partitions N`, các dataset `price`, `index_price`, `price_board` chỉ đọc **N partition `trading_date` mới nhất** — DAG `structured_daily` dùng `N=7` để tránh upsert lại toàn bộ lịch sử. Incremental thật vẫn nằm ở Bronze ingest và Silver transform.
...
- **Phụ thuộc luồng news:** cần `silver.news` + `mart_stock_news_signal` (từ DAG `news_daily`) để cột sentiment đầy đủ; nếu chưa ingest news, join vẫn chạy, sentiment NULL/0.
- Index: `(ticker, trading_date)` unique, `(trading_date)`.
- Grain: `ticker + trading_date` (Timescale hypertable theo DDL nếu đã apply).
- Snapshot (tích lũy nhiều `trading_date`; ví dụ partition `2026-06-03`: 557 rows).
...
- **Top movers:** JSON `top_gainers` / `top_losers` (top 5 theo `daily_return` mỗi ngày), tính bằng `row_number()` theo `trading_date` (không dùng correlated subquery).
- Index: `trading_date` (unique).
- Grain: **1 dòng / trading_date**. Snapshot demo: **1,247** rows.

### 9.8. DAG phụ thuộc (dbt lineage + Airflow)

**Orchestration Airflow:** `structured_daily` chạy `price + index + price_board`; `structured_monthly` chạy `listing + company + financial_ratio`. DAG `gold_full_refresh` (19:00 hàng ngày) chạy full `dbt run` + `dbt test`.
...
### 9.10. Snapshot row counts (workspace local, 2026-06-03)

| Layer | Object | Rows (tham chiếu) | Ghi chú |
|---|---|---:|---|
| Silver parquet | `listing/current` | 1,532 | |
| Silver parquet | `company/current` | 703 | Full listing universe |
| Silver parquet | `price_board` (`2026-06-03`) | 703 | Dedupe Bronze; mặc định ingest **1** `snapshot_at`/run (50 mã) |
| Silver parquet | `price` (`2026-06-03`) | 557 | Tích lũy nhiều `trading_date` nếu backfill |
| Silver parquet | `news` (`2026-06-03`) | 924 | |
| Silver parquet | `bctc_pdf_meta` (`2026-06-03`) | 1,867 | |
| Gold (sau `dbt run`) | `mart_stock_daily` | (theo PG) | Rebuild sau load Silver |
| Gold (sau `dbt run`) | `mart_company_profile` | (theo PG) | |
| Gold (sau `dbt run`) | `mart_price_board` | (theo PG) | API `/board/{symbol}` |
...
|---|---|---|---|
| Search ticker | `GET /tickers` (search) | `mart_ticker_directory` | Union mã có price/news/BCTC (~700+ sau full run) |
...
```powershell
# 1. Bronze
# Khi use_listing_as_universe=True (mặc định), listing chạy tự động trước price
# trong pipeline — không cần chạy thủ công riêng.
@'
from ingestion.structure_data import IngestionConfig, run_structure_ingestion_pipeline
...
| `max_tickers_per_run` | 50 | Giới hạn khi **không** dùng listing universe; với `use_listing_as_universe=True` pipeline dùng toàn bộ listing đã filter |
| `rate_limit_rpm` | 50 | Rate limit toàn cục |
| `delay_between_categories_sec` | 30 | Nghỉ giữa nhóm API trong pipeline |
| `use_listing_as_universe` | `True` | Dùng Bronze listing làm ticker universe sau ingest listing |
| `listing_exchange_filter` | `["HOSE","HNX"]` | Filter exchange (rỗng = tất cả); mặc định bỏ UPCOM |
| `listing_security_type_filter` | `[]` | Filter loại CK (rỗng = tất cả); cột Bronze `type` / `security_type` |
| `price_batch_size` | `100` | Số tickers/batch khi fetch OHLCV |
| `price_board_use_listing_universe` | `True` | Full listing; `False` = watchlist |
| `price_board_max_tickers` | `5000` | Cap số mã price board |
| `price_board_batched` | `True` | Nhiều request API, gộp một `snapshot_at` |
| `price_board_tickers` | `DEFAULT_PRICE_BOARD_TICKERS` | Chỉ dùng khi `use_listing_universe=False` |
| `price_board_batch_size` | `50` | Kích thước chunk khi `price_board_batched=True` |
| `delay_between_batches_sec` | `5.0` | Nghỉ giữa các batch (giây) |
| `financial_ratio_exchange_filter` | `["HOSE","HNX"]` | Chỉ fetch ratio cho sàn này; rỗng = tất cả |
...
- [ingestion/ingest_structure_data_manager.ipynb](../ingestion/ingest_structure_data_manager.ipynb) — `UNIVERSE_MODE`, `RUN_PROFILE`, `RUN_ONE_SHOT_PIPELINE`
...
## 15. Current Structured/API/UI Behavior (2026-06-03)
...
### 15.4 Financial annual-row logic (exact)

`gold.mart_financial_summary` is built in two layers:

1. `period_rows`: pivot metrics by `ticker + period + period_type + year + quarter`.
2. `annual_rows`: for each `ticker + year`, pick the latest available quarter (`row_number() over (partition by ticker, year order by quarter desc nulls last, period desc) = 1`) and remap it to:
   - `period_type = 'annual'`
   - `period = year::text`
   - `quarter = null`

Final output is `period_rows UNION ALL annual_rows`.
This is why the annual tab can show values even when raw annual rows are missing in upstream ratio data.
