...
| `hnx_max_list_pages` (field config) | Mặc định class **10** (DAG weekly; trang 1 = mới nhất). Notebook backfill: **500** qua `BCTC_RUN_PROFILE` hoặc truyền vào constructor |
...
2. **Profile crawl (`BCTC_RUN_PROFILE`):**
   ...
   | `dag_weekly` | **10** | Thử nghiệm giống DAG; code CLI không set gì cũng ra **10** |
...
| Bước | Notebook backfill | DAG `bctc_weekly` |
|---|---|---|
...
| Bronze metadata | `data-lake/raw/Semi_Structure_Data/bctc_annual_pdf_meta/source=hnx/date=<run_date>/PART-000.parquet` | Cùng layout, mỗi tuần một `run_partition` mới |
...
- `bctc_weekly` hiện là **quét lại top 10 trang gần nhất rồi upsert theo `doc_id`**, chưa phải incremental crawl theo watermark ngày công bố.
- Partition Bronze/Silver là **ngày chạy job**, không phải `published_at`.
- `incremental_window_days=30` hiện có trong config/notebook nhưng chưa điều khiển provider crawl HNX.
...
| **DAG `bctc_weekly` (Airflow)** | `hnx_max_list_pages=10` (mặc định class) — T7 10:00 ICT; file `docker/airflow/dags/bctc_weekly.py`; dbt `+mart_bctc_documents`. Không set `HNX_CRAWL_MAX_LIST_PAGES=500` trừ backfill thủ công. |
| **Lần chạy tiếp theo** | **Cùng cơ chế full crawl** theo số trang đã cấu hình (không đọc `incremental_window_days`). Metadata mới nằm partition **`date=` ngày chạy mới**. Upsert warehouse/Silver theo `doc_id` khi load — trùng `doc_id` thì cập nhật. |
...
**Orchestration:** DAG `bctc_weekly` — `bronze_bctc` → `silver_bctc` (`bctc_pdf_meta`, `--run-partition` từ XCom) → `load_silver` → `dbt_marts`.
