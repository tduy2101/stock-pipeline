...
Dự án **stock-pipeline** (xem [README.md](../README.md)) là **Medallion Data Architecture** cho chứng khoán Việt Nam: Bronze gần nguồn, Silver parquet sạch, PostgreSQL warehouse phân tích, dbt Gold, FastAPI read-only, React dashboard. Đây là batch ELT / analytical serving layer, không phải hệ thống OLTP chính.
...
Tên module/path hiện vẫn dùng `bctc_annual_*` vì ban đầu ưu tiên báo cáo tài chính năm; DAG vận hành hiện tại là `bctc_quarterly`, quét lại các công bố HNX gần nhất theo quý và upsert metadata theo `doc_id`.
...
| `hnx_max_list_pages` (field config) | Mặc định class **10** (DAG quarterly; trang 1 = mới nhất). Notebook backfill: **500** qua `BCTC_RUN_PROFILE` hoặc truyền vào constructor |
| `HNX_CRAWL_MAX_LIST_PAGES` | Env tùy chọn — **ghi đè** `hnx_max_list_pages` khi set (backfill: `500`; test: `1`) |
...
2. **Profile crawl (`BCTC_RUN_PROFILE`):**

   | Profile | `hnx_max_list_pages` | Khi dùng |
   |---|---:|---|
   | `backfill` (mặc định notebook) | **500** | Lần đầu / backfill — tối đa cho phép trong config |
   | `dag_quarterly` | **10** | Thử nghiệm giống DAG; code CLI không set gì cũng ra **10** |

   Cell profile gán `HNX_MAX_LIST_PAGES`; cell config truyền `hnx_max_list_pages=HNX_MAX_LIST_PAGES` vào `SemiStructuredIngestionConfig`. Có thể thay bằng env `HNX_CRAWL_MAX_LIST_PAGES` (ghi đè khi resolve).

3. **Cấu hình:** `configure_logging()`, `load_dotenv_from_project_root()`, tạo `SemiStructuredIngestionConfig`:

   | Tham số | Giá trị notebook (backfill) | Ý nghĩa |
   |---|---|---|
   | `sources` | `["hnx"]` | Nguồn duy nhất phase hiện tại |
   | `hnx_max_list_pages` | `500` (từ profile) | Số trang POST list HNX |
...
4. **Chạy:** `out_download = run_bctc_annual_pipeline(cfg, include_download=True)` — log crawl: `HNX list crawl: max_pages=...`.
5. **Kiểm tra:** đọc parquet metadata, group theo `ticker`, đếm `status`.

**Lần đầu:** `BCTC_RUN_PROFILE="backfill"`, **không** bật `HNX_RESUME_FROM_STATE`. **DAG sau này:** `SemiStructuredIngestionConfig()` (10 trang), không set env trang; có thể bật resume state tùy vận hành.
...
### 4.3. Tóm tắt backfill -> DAG cho BCTC

| Bước | Notebook backfill | DAG `bctc_quarterly` |
|---|---|---|
| Phạm vi crawl | `BCTC_RUN_PROFILE=backfill` -> `hnx_max_list_pages=500` trong code hiện tại | `SemiStructuredIngestionConfig()` -> `hnx_max_list_pages=10` |
| Bronze metadata | `data-lake/raw/Semi_Structure_Data/bctc_annual_pdf_meta/source=hnx/date=<run_date>/PART-000.parquet` | Cùng layout, mỗi quý một `run_partition` mới |
| Bronze PDF | `data-lake/raw/Semi_Structure_Data/bctc_annual_pdf/source=hnx/date=<run_date>/ticker=<TICKER>/year=<YYYY>/<doc_id>.pdf` | Cùng layout |
| Silver | `data-lake/silver/bctc_pdf_meta/date=<run_date>/PART-000.parquet` | Cùng layout; task silver đọc đúng `run_partition` bronze vừa sinh ra |
| Warehouse / Gold | `silver.bctc_pdf_meta` -> `gold.mart_bctc_documents` | Cùng luồng; API/UI chỉ đọc sau load + dbt |

Điểm quan trọng:

- `bctc_quarterly` hiện là **quét lại top 10 trang gần nhất rồi upsert theo `doc_id`**, chưa phải incremental crawl theo watermark ngày công bố.
- Partition Bronze/Silver là **ngày chạy job**, không phải `published_at`.
...
| **DAG `bctc_quarterly` (Airflow)** | `hnx_max_list_pages=10` (mặc định class) — 10:00 ICT ngày 15 các tháng 2/5/8/11; file `docker/airflow/dags/bctc_quarterly.py`; dbt `+mart_bctc_documents`. Không set `HNX_CRAWL_MAX_LIST_PAGES=500` trừ backfill thủ công. |
| **Lần chạy tiếp theo** | **Cùng cơ chế full crawl** theo số trang đã cấu hình. Metadata mới nằm partition **`date=` ngày chạy mới**. Upsert warehouse/Silver theo `doc_id` khi load — trùng `doc_id` thì cập nhật. |
...
**Kết luận vận hành:** Coi mỗi job Bronze BCTC là **snapshot crawl + download theo ngày chạy** (`run_date`). “Incremental” ở tầng warehouse là **upsert theo `doc_id`**, không phải crawl chỉ bài mới.
...
```text
Bronze crawl (một run)     →  ~1,800+ dòng metadata (mọi status; ví dụ 2026-06-03: 1,867)
...
| Layer | Rows (workspace 2026-06-03) | Ghi chú |
|---|---:|---|
| `silver.bctc_pdf_meta` (`date=2026-06-03`) | **1,867** | Mọi tài liệu crawl (mọi `status`) |
| **`gold.mart_bctc_documents`** | (sau `dbt run`) | Subset web-ready; demo cũ ~**952** |

Chênh lệch Bronze → Gold ≈ `download_skipped_filter` + failed/skipped (giải trình EN, `explanation`, filter BCTC VI, v.v.).

**Log vận hành thường gặp (không phải lỗi rate limit):** `ingest_skip | reason=skipped_doc_class:explanation`, `skipped_language_en` — filter ingest; `download_attempt status=200` = tải PDF OK.

### 9.7. DAG phụ thuộc (dbt lineage + Airflow)

**Orchestration:** DAG `bctc_quarterly` — `bronze_bctc` → `silver_bctc` (`bctc_pdf_meta`, `--run-partition` từ XCom) → `load_silver` → `dbt_marts`.
...
| `hnx_max_list_pages` | `10` | Notebook backfill: `500` (`BCTC_RUN_PROFILE`); env `HNX_CRAWL_MAX_LIST_PAGES` ghi đè |
